...
fn done_menu() -> MenuState {
    MenuState::new("Indexing complete", vec![
        MenuItem::new("Done", "return to previous screen"),
        MenuItem::new("Re-index", "rebuild embeddings"),
    ])
}
...
impl EmbedScreen {
    pub fn new() -> Self {
        Self { phase: Phase::Confirm(Confirm::new("Index corpus into semantic search?", true)), poll_pending: None, start_pending: None }
    }

    fn start_job(&mut self, ctx: &mut AppContext) {
        self.phase = Phase::Running(ProgressBar::new("Indexing Embeddings"), String::new());
        let api = ctx.api_url.clone();
        self.start_pending = Some(tokio::spawn(async move {
            let client = crate::api::client::ApiClient::new(&api);
            client.start_embed("corpus/clean/tort/corpus.json").await
        }));
    }
...
impl Screen for EmbedScreen {
    fn name(&self) -> &str { "Index Semantic Search" }
