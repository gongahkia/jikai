...
pub struct EmbedScreen { phase: Phase }

impl EmbedScreen {
    pub fn new() -> Self {
        Self { phase: Phase::Confirm(Confirm::new("Index corpus into semantic search?", true)) }
    }
}
        ...
        match &mut self.phase {
            Phase::Confirm(c) => {
                if let Some(yes) = c.handle_key(key) {
                    if yes {
                        self.phase = Phase::Running(Spinner::new("Indexing embeddings..."));
                        let api = ctx.api_url.clone();
                        tokio::spawn(async move {
                            let client = crate::api::client::ApiClient::new(&api);
                            let _ = client.start_embed("corpus/clean/tort/corpus.json").await;
                        });
