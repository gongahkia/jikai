...
fn done_menu(fmt: &str) -> MenuState {
    MenuState::new(
        &format!("Exported as {}", fmt.to_uppercase()),
        vec![
...
impl ExportScreen {
    pub fn new() -> Self {
        Self {
            phase: Phase::FormatSelect(format_menu()),
            poll_pending: None,
            start_pending: None,
        }
    }
    ...
    fn start_export(&mut self, format: &str, ctx: &mut AppContext) {
        self.phase = Phase::Running(
            ProgressBar::new(&format!("Exporting {}", format.to_uppercase())),
            String::new(),
        );
            ...
            crate::async_join::take_join_result_if_finished(&mut self.poll_pending)
        {
            match result {
                Ok(Ok(status)) => match status.status.as_str() {
                    "completed" => {
                        self.phase = Phase::Done(done_menu("file"));
                    }
                    "failed" => {
