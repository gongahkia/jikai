...
pub struct LabelScreen {
    ...
    pending:
        Option<tokio::task::JoinHandle<Result<Vec<crate::api::types::CorpusEntry>, anyhow::Error>>>,
}

fn done_menu(count: usize) -> MenuState {
    MenuState::new(
        &format!("Labelled {} entries", count),
        vec![
...
impl LabelScreen {
    pub fn new() -> Self {
        Self {
            phase: Phase::Loading(Spinner::new("Loading corpus...")),
            entries: Vec::new(),
            labels: Vec::new(),
            pending: None,
        }
    ...
    fn show_entry(&mut self, idx: usize) {
        if let Some(entry) = self.entries.get(idx) {
            let panel = Panel::new(
                &format!("Entry {}/{}", idx + 1, self.entries.len()),
                &entry.text,
            );
            let cb = CheckboxState::new(
                "Label topics (Space=toggle, Enter=confirm)",
                topics::topic_items(),
            );
            self.phase = Phase::Labelling {
                entry_idx: idx,
                panel,
                topics_cb: cb,
            };
