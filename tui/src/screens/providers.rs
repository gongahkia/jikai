...
pub struct ProvidersScreen {
    ...
    pending: Option<
        tokio::task::JoinHandle<Result<(serde_json::Value, serde_json::Value), anyhow::Error>>,
    >,
}
...
impl ProvidersScreen {
    pub fn new() -> Self {
        Self {
            phase: Phase::Loading(Spinner::new("Checking providers...")),
            health_data: None,
            models_data: None,
            pending: None,
        }
        ...
        match &mut self.phase {
            ...
            Phase::Detail(text) => {
                let block = Block::default()
                    .title(Span::styled(" Provider Detail ", theme::title()))
                    .borders(Borders::ALL)
