...
impl StatsScreen {
    pub fn new() -> Self {
        Self {
            phase: Phase::Loading(Spinner::new("Loading statistics...")),
            pending: None,
        }
    }
}
...
impl Screen for StatsScreen {
    ...
    fn handle_key(&mut self, key: KeyEvent, ctx: &mut AppContext) -> ScreenAction {
        match &mut self.phase {
            ...
            Phase::Display(_) => match key.code {
    ...
    fn tick(&mut self, _ctx: &mut AppContext) {
        ...
        if let Some(result) = crate::async_join::take_join_result_if_finished(&mut self.pending) {
            match result {
                Ok(Ok(stats)) => {
                    self.phase = Phase::Display(
                        serde_json::to_string_pretty(&stats).unwrap_or_else(|_| "No data".into()),
                    );
                }
                Ok(Err(e)) => {
                    let msg = format!("{}", e);
