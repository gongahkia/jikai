...
impl DataTable {
    pub fn new(title: &str, headers: Vec<String>, widths: Vec<u16>) -> Self {
        Self { title: title.into(), headers, rows: Vec::new(), widths, state: TableState::default() }
    }
    ...
    fn move_down(&mut self) {
        let i = self.state.selected().unwrap_or(0);
        if i + 1 < self.rows.len() { self.state.select(Some(i + 1)); }
    }

    pub fn render(&mut self, f: &mut Frame, area: Rect) {
        let header = Row::new(self.headers.iter().map(|h| Span::styled(h.as_str(), theme::header())));
        let widths: Vec<ratatui::layout::Constraint> = self.widths.iter().map(|&w| ratatui::layout::Constraint::Length(w)).collect();
        let rows: Vec<Row> = self.rows.iter().map(|row| {
            Row::new(row.iter().map(|cell| Span::raw(cell.as_str())))
        }).collect();
