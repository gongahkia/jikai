...
impl Confirm {
    pub fn new(message: &str, default_yes: bool) -> Self {
        Self { message: message.into(), yes_selected: default_yes }
    }
    ...
    pub fn handle_key(&mut self, key: KeyEvent) -> Option<bool> {
        match key.code {
            ...
            KeyCode::Left | KeyCode::Char('h') => { self.yes_selected = true; None }
            KeyCode::Right | KeyCode::Char('l') => { self.yes_selected = false; None }
    ...
    pub fn render(&self, f: &mut Frame, area: Rect) {
        let yes_style = if self.yes_selected { theme::selected() } else { theme::normal() };
        let no_style = if !self.yes_selected { theme::selected() } else { theme::normal() };
