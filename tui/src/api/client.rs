...
impl ApiClient {
    ...
    pub async fn select_provider(&self, name: &str) -> Result<serde_json::Value> {
        Ok(self
            .client
            .post(self.url("/llm/select-provider"))
    ...
    pub async fn select_model(&self, name: &str) -> Result<serde_json::Value> {
        Ok(self
            .client
            .post(self.url("/llm/select-model"))
            .json(&serde_json::json!({"name": name}))
            .send()
            .await?
            .json()
            ...
            if let Ok(err) = serde_json::from_str::<serde_json::Value>(&text) {
                if let Some(detail) = err.get("detail") {
                    let api_err: ApiError =
                        serde_json::from_value(detail.clone()).unwrap_or(ApiError {
                            code: "unknown".into(),
                            message: text.clone(),
                            hint: String::new(),
                            retryable: false,
    ...
    pub async fn list_reports(&self, generation_id: i64) -> Result<Vec<GenerationReport>> {
        let resp: serde_json::Value = self
            .client
            .get(self.url(&format!("/workflow/reports/{}", generation_id)))
            .send()
            .await?
            .json()
            .await?;
    ...
    pub async fn get_history(&self, limit: u32) -> Result<Vec<HistoryRecord>> {
        let resp: serde_json::Value = self
            .client
            .get(self.url(&format!("/db/history?limit={}", limit)))
            .send()
            .await?
            .json()
            .await?;
    ...
    pub async fn get_generation(&self, id: i64) -> Result<serde_json::Value> {
        Ok(self
            .client
            .get(self.url(&format!("/db/generation/{}", id)))
            .send()
            .await?
            .json()
            .await?)
    ...
    pub async fn validate(&self, req: &ValidateRequest) -> Result<serde_json::Value> {
        Ok(self
            .client
            .post(self.url("/validation/validate"))
            .json(req)
            .send()
            .await?
            .json()
    ...
    pub async fn start_preprocess(
        ...
        self.start_preprocess_with_options(raw_dir, output_path, true, false)
            .await
    }
    ...
    pub async fn start_train(&self, data_path: &str, n_clusters: u32) -> Result<String> {
        self.start_train_with_options(data_path, n_clusters, None)
            .await
    }
    ...
    pub async fn start_embed_with_options(
        &self,
        corpus_path: &str,
        batch_size: u32,
    ) -> Result<String> {
    ...
    pub async fn job_status(&self, job_id: &str) -> Result<JobStatus> {
        Ok(self
            .client
            .get(self.url(&format!("/jobs/{}/status", job_id)))
            .send()
            .await?
            .json()
            .await?)
