...
class TopicSelector:
    ...
    def select(
        ...
        """
        if not self._corpus_entries or self._tfidf_matrix is None:
            return []
        ...
        for i, entry in enumerate(self._corpus_entries):
            ...
            if classifier and hasattr(classifier, "is_trained") and classifier.is_trained:
                try:
                    ...
                    X, _ = extract_features([entry["text"]], vectorizer=self._vectorizer)
                    pred = classifier.model.predict(X)
                    label_names = getattr(classifier, "label_names", [])
                    if label_names is not None:
                        predicted_topics = {label_names[j] for j in range(len(label_names)) if pred[0][j] == 1}
                        classifier_score = len(predicted_topics & requested_set) / max(len(requested_set), 1)
                except Exception:
                    pass
            combined = 0.4 * tfidf_scores[i] + 0.4 * topic_score + 0.2 * classifier_score
