...
def load_data(csv_path: str, test_size: float = 0.2, random_state: int = 42,
    ...
    df["topic_list"] = df["topic_labels"].apply(lambda x: [t.strip() for t in str(x).split("|")])
    if progress_callback:
...
def extract_features(texts, max_features: int = 5000, ngram_range: Tuple[int, int] = (1, 2),
...
def binarize_labels(topic_lists, binarizer: Optional[MultiLabelBinarizer] = None) -> Tuple:
