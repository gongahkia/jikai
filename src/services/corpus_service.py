...
class CorpusService:
    ...
    async def _load_from_local(self) -> List[HypotheticalEntry]:
        ...
        if not self._local_corpus_path.exists():
            raise CorpusServiceError(f"Local corpus file not found: {self._local_corpus_path}")
    ...
    async def save_corpus(self, entries: List[HypotheticalEntry], destination: str = "local") -> bool:
    ...
    async def add_hypothetical(self, entry: HypotheticalEntry, destination: str = "local") -> str:
        ...
        try:
            ...
            logger.info("Hypothetical added to corpus", id=entry.id, topics=entry.topics)
            return entry.id
    ...
    async def update_hypothetical(self, entry_id: str, updates: Dict[str, Any], destination: str = "local") -> bool:
        ...
        try:
            ...
            logger.info("Hypothetical updated", id=entry_id, updates=list(updates.keys()))
            return True
