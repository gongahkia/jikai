...
_CHINESE_NAMES = [
    "Tan Wei Ming", "Lim Shu Fen", "Ng Kai Lun", "Wong Mei Ling", "Chen Jia Hao",
    "Lee Xin Yi", "Goh Zhi Wei", "Ong Hui Min", "Koh Boon Kiat", "Chua Li Ting",
]
_MALAY_NAMES = [
    "Ahmad bin Ismail", "Siti binti Rahman", "Mohd Faizal", "Nurul Ain", "Razak bin Osman",
    "Hakim bin Yusof", "Aishah binti Kadir", "Farhan bin Zain", "Zulaikha binti Hassan",
]
_INDIAN_NAMES = [
    "Rajesh Nair", "Priya Devi", "Suresh Kumar", "Anitha Pillai", "Vikram Rajan",
    "Deepa Menon", "Arjun Krishnan", "Kavitha Sundaram", "Ganesh Iyer",
]
_WESTERN_NAMES = [
    "James Mitchell", "Sarah Collins", "David Thompson", "Emma Richardson", "Michael Stuart",
    "Rachel Pemberton", "Andrew Fielding", "Catherine Blake",
]
...
_LOCATIONS = [
    "Orchard Road", "Tampines", "Jurong East", "Toa Payoh", "Bukit Timah",
    "Marine Parade", "Ang Mo Kio", "Clementi", "Bedok", "Woodlands",
    "Hougang", "Sentosa", "Marina Bay", "Chinatown", "Little India",
]
_STRUCTURAL_SLOTS = [
    "scenario_setup", "parties_introduction", "factual_complications",
    "legal_issues", "analysis_framework",
]
...
class PartyNameGenerator:
    ...
    def generate(self, count: int = 3, include_company: bool = True) -> List[str]:
        ...
        if include_company and count >= 2:
            base = self._rng.choice(["Alpha", "Bright", "Coastal", "Delta", "Eastern", "Fortune", "Global"])
...
class CorpusRetriever:
    ...
    def retrieve(self, topics: List[str], n_results: int = 5, classifier=None) -> List[Dict]:
        if self._topic_selector is None:
            return []
        results = self._topic_selector.select(topics, n_results=n_results, classifier=classifier)
...
class TemplateEngine:
    ...
    def get_template(self, topics: List[str], complexity: int) -> Dict[str, str]:
        ...
        scenario_patterns = tmpl_data.get("scenario_patterns", ["A dispute arises in Singapore."])
        ...
        template = {
            ...
            "legal_issues": "The following legal issues arise: " + ", ".join(topics) + ".",
            "analysis_framework": "",
        }
        if legal_tests:
            template["legal_issues"] += " Key legal tests: " + "; ".join(legal_tests[:3]) + "."
        if defences and complexity >= 3:
            template["factual_complications"] = "Available defences include: " + ", ".join(defences[:2]) + "."
        return template
...
class HypoAssembler:
    ...
    def assemble(
        ...
        if complexity_constraints and hasattr(complexity_constraints, "word_count_min"):
            ...
            if word_count < complexity_constraints.word_count_min:
                text = self._expand_text(text, corpus_fragments, complexity_constraints.word_count_min - word_count)
        return text
    ...
    def _build_party_intro(self, names: List[str], location: str) -> str:
        ...
        roles = ["operates a business", "lives nearby", "works as a professional", "is a regular visitor", "manages the premises"]
        for i, name in enumerate(names):
            role = roles[i % len(roles)]
            lines.append(f"{name} {role} in the area.")
        return " ".join(lines)

    def _extract_fact_patterns(self, fragments: List[Dict], party_names: List[str]) -> str:
...
class VariationEngine:
    ...
    _SEVERITY_MODIFIERS = [
        ("minor", "serious"), ("slight", "severe"), ("small", "significant"),
        ("temporary", "permanent"), ("mild", "critical"),
    ]
    _DEFENCE_ADDITIONS = [
    ...
    def _swap_party_roles(self, text: str) -> str:
        names = re.findall(r'\b[A-Z][a-z]+(?:\s(?:bin|binti|Pte|Ltd)\s)?[A-Z]?[a-z]*\b', text)
    ...
    def _change_severity(self, text: str) -> str:
        for mild, severe in self._SEVERITY_MODIFIERS:
            if mild in text.lower():
                text = re.sub(rf'\b{mild}\b', severe, text, count=1, flags=re.IGNORECASE)
                break
        return text
...
class QualityGate:
    ...
    def evaluate(self, text: str) -> Dict[str, float]:
        ...
        if pipeline is None or not pipeline.regressor.is_trained or pipeline._vectorizer is None:
            return {"overall": 0.7, "topic_coverage": 0.7, "complexity_match": 0.7, "structural_completeness": 0.7}
...
class HypoGenerator:
    ...
    async def initialize(self):
        ...
        try:
            ...
            corpus_dicts = [{"text": e.text, "topics": e.topics, "id": e.id} for e in entries]
    ...
    async def generate(
        ...
        plan = planner.predict("", topics, complexity) if planner else {
        ...
        for attempt in range(max_retries):
            ...
            text = self.assembler.assemble(template, fragments, party_names, location, constraints)
            ...
            if not best_text or scores.get("overall", 0) > best_scores.get("overall", 0):
                best_text = text
                best_scores = scores
            if passes:
                break
